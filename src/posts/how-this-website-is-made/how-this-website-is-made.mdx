...
Another thing that I like is the better separation between server and client side. By default all components runs on the server and if you want to perform some client operation you write *'use client'* on top of the file and this feels way more declarative. Debugger is also pretty good and able to spot if you're mixing server and client code and it's easier to avoid nasty *hydrations errors*.

I'm a little bit confused when it comes to the new *Layout* instead. I mean, since we have server components what prevents me to define and use my custom layout components instead? I think this is a legit question because of some limitations the built-in Layout comes with.

But yeah, it's been a good overall experience until now and working with it feels pretty fun.
